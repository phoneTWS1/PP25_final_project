...
# 2. 程式和檔案名稱
DATA_DIR="../dataset"
STRASSEN_EXE="./strassen_seq" # 假設 Makefile 會生成這個名稱的執行檔
...
make clean
make strassen_seq # 或者使用 'make all'，取決於您的 Makefile
if [ $? -ne 0 ]; then 
...
# 檢查執行檔是否存在
if [ ! -f "$STRASSEN_EXE" ]; then
    echo "錯誤: Makefile 沒有生成預期的執行檔 $STRASSEN_EXE，請檢查 Makefile 的輸出名稱。" | tee -a $LOG_FILE
    exit 1
fi
...
for N in "${TEST_DIMENSIONS[@]}"; do
    ...
    "$STRASSEN_EXE" "$N" "$A_FILE" "$B_FILE" "$C_TRUE_FILE" 2>&1 | tee -a $LOG_FILE
