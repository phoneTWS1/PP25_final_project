...
int main(int argc, char **argv){
    ...
    for(size_t i=0; i<sz; i++){
        A[i] = (float)rand() / (float)RAND_MAX; // A 是一個隨機矩陣
        B[i] = (float)rand() / (float)RAND_MAX; // B 是另一個不同的隨機矩陣
    }
    ...
    // 5. 寫入檔案
    if(fwrite(A, sizeof(float), sz, fa) != sz) { perror("fwrite A"); }
    if(fwrite(B, sizeof(float), sz, fb) != sz) { perror("fwrite B"); }
    if(fwrite(C, sizeof(float), sz, fc) != sz) { perror("fwrite C"); }
    
    // 6. 清理
    fclose(fa); fclose(fb); fclose(fc);
    free(A); free(B); free(C);

    printf("Successfully wrote input: %s, %s and answer: %s (binary float32, row-major)\n", fnameA, fnameB, fnameC);
    return 0;
}